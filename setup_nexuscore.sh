...
if [ "$ENABLE_PASSWORD_AUTH" = true ]; then
    ...
    set_password_auth() {
        ...
        if grep -qE "^\s*PasswordAuthentication\s+yes" "$file_to_check"; then
            ...
            backup_file "$file_to_check"
            sudo sed -i 's/^\s*#\s*PasswordAuthentication\s+yes/PasswordAuthentication yes/' "$file_to_check"
            PASSWORD_AUTH_SET=true; AUTH_TARGET_FILE="$file_to_check"; return 0;
        elif grep -qE "^\s*PasswordAuthentication\s+no" "$file_to_check"; then
            backup_file "$file_to_check"
            sudo sed -i 's/^\s*PasswordAuthentication\s+no/PasswordAuthentication yes/' "$file_to_check"
            PASSWORD_AUTH_SET=true; AUTH_TARGET_FILE="$file_to_check"; return 0;
        fi
        return 1
    }
    ...
    log_info "Skipping SSH password authentication setup as per configuration."
fi
